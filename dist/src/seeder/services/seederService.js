...
import { buildExpertiseForSpeciality, buildExpertiseEnForSpeciality, buildTimelineForDoctor, } from './doctorInformationContent.js';
...
function feeForChamber(base, ci) {
    const bump = ci * 100;
...
function buildDoctors() {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        const chamberSlugs = [HOSPITALS[homeIdx].slug];
        if (i % 5 === 0) {
            let otherIdx = (homeIdx + 3) % HOSPITALS.length;
            if (otherIdx === homeIdx)
                otherIdx = (otherIdx + 1) % HOSPITALS.length;
...
export const runSeedAll = async () => {
    ...
    for (const d of allDoctors) {
        ...
        await prisma.doctorInformation.upsert({
            where: { doctorId: d.id },
            update: { expertise: expertise, expertise_en: expertise_en, timeline: timeline },
            create: {
                doctorId: d.id,
                expertise: expertise,
                ...
                highlights_en: DEFAULT_HIGHLIGHTS_EN,
                stats: DEFAULT_STATS,
                stats_en: DEFAULT_STATS_EN,
            },
        });
        infoCount++;
        infoIndex++;
    ...
    let poolIndex = 0;
    for (const d of allDoctors) {
        for (const s of buildDoctorPoolSeeds({ username: d.username, name: d.name, speciality: d.speciality }, (poolIndex * 7) % 60)) {
            await prisma.blog.upsert({
                where: { slug: s.slug },
                update: {
                    title: s.title,
