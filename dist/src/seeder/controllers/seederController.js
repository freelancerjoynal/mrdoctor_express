...
export const seedAll = async (_req, res) => {
    try {
        ...
        return res.status(201).json({
            success: true,
            message: 'নীলফামারী জেলার ১০ হাসপাতাল, ১০০ ডাক্তার, চেম্বার (per-chamber ফি সহ) ও সিডিউল সফলভাবে সিড করা হয়েছে এবং ইউজার অ্যাকাউন্টের সাথে যুক্ত করা হয়েছে!',
            summary,
        });
    }
    catch (error) {
...
export const seedSuperAdminPublic = async (req, res) => {
    try {
        ...
        const seedRole = role === 'ADMIN_MANAGER' ? 'ADMIN_MANAGER' : 'SUPER_ADMIN';
        ...
        const msg = error.message || 'Failed to seed admin';
...
export const seedAdminManager = async (req, res) => {
    try {
        ...
        const msg = error.message || 'Failed to seed admin manager';
        const status = msg.startsWith('INVALID_') || msg.startsWith('PASSWORD_') ? 400 : 500;
        return res.status(status).json({ success: false, error: 'Failed to seed admin manager', details: msg });
    }
};
//# sourceMappingURL=seederController.js.map