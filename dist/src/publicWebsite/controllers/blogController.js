...
export const listDoctorBlogs = async (req, res) => {
    try {
        const rawTake = Array.isArray(req.query.take) ? req.query.take[0] : req.query.take;
        const blogs = await getPublicDoctorBlogs(req.params.username, typeof rawTake === 'string' ? parseInt(rawTake, 10) || 6 : 6);
        if (!blogs)
...
export const listHospitalBlogs = async (req, res) => {
    try {
        const rawTake = Array.isArray(req.query.take) ? req.query.take[0] : req.query.take;
        const blogs = await getPublicHospitalBlogs(req.params.slug, typeof rawTake === 'string' ? parseInt(rawTake, 10) || 6 : 6);
        if (!blogs)
