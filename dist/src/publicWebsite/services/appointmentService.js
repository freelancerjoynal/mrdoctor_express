...
export function buildDayLabel(date) {
    const now = new Date();
...
function cleanProblem(raw) {
    ...
    if (problem.length < 3 || problem.length > 1000)
        throw new Error('INVALID_PROBLEM');
    return problem;
}
/** Booking options for the form: chambers + schedules + next running days. */
export async function getAppointmentOptions(username) {
    ...
    const ownerOf = (dayOfWeek) => doctor.schedules.find((s) => s.chamberId && String(s.dayOfWeek).toUpperCase() === dayOfWeek)?.chamberId ?? null;
    const days = [];
    const now = new Date();
    for (let offset = 0; offset < 30 && days.length < 2; offset++) {
        const date = new Date(now.getFullYear(), now.getMonth(), now.getDate() + offset);
        const dayOfWeek = jsDayToEnum(date);
        if (!roster.includes(dayOfWeek))
            continue;
...
export async function getSerialLiveBoard(username) {
    const key = (username || '').trim();
    if (!key)
        throw new Error('DOCTOR_NOT_FOUND');
    const doctor = await prisma.doctor.findFirst({
    ...
    if (!doctor)
        throw new Error('DOCTOR_NOT_FOUND');
    const breakUntilRaw = doctor.liveBreakUntil;
    const breakUntil = breakUntilRaw instanceof Date
        ? breakUntilRaw
        : typeof breakUntilRaw === 'string'
            ? new Date(breakUntilRaw)
            : null;
    const breakActive = doctor.serialLive &&
    ...
    if (!doctor.serialLive)
        return { ...base, break: null, current: null, next: null, upcoming: [], missed: [], waitingCount: 0, totalToday: 0 };
...
export async function createAppointment(input) {
    ...
    const patientArea = typeof input.patientArea === 'string' && input.patientArea.trim()
    ...
    const scoped = slugKey
        ...
        : doctor.chambers;
    const ownerOfDay = (day) => doctor.schedules.find((s) => s.chamberId && String(s.dayOfWeek).toUpperCase() === day)?.chamberId ?? null;
