...
export const HOSPITAL_TEXTS = {
    ...
    DEPT_DOCTORS_HEADER: (hospitalName, dept) => `👨‍⚕️ *${hospitalName}* — *${dept}* বিভাগের ডাক্তারগণ:\n`,
...
export function getHospitalFaqAnswer(normText) {
...
export function buildTrackingSummary(data) {
    const parts = ["🏥 *হসপিটাল খোঁজার অনুরোধ*"];
    if (data.location)
        parts.push(`📍 এলাকা: ${data.location}`);
    if (data.hospitalName)
        parts.push(`🏥 হসপিটাল: ${data.hospitalName}`);
    if (data.department)
        parts.push(`🏷️ বিভাগ: ${data.department}`);
    return parts.join("\n");
}
//# sourceMappingURL=hospitalQA.js.map