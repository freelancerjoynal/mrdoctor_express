...
export const DOCTOR_TEXTS = {
    ...
    welcomeMessage: (doctorName) => `আসসালামু আলাইকুম, আমি ${doctorName} বলছি`,
    chamberLocation: (places) => `আমার চেম্বারের ঠিকানা:\n${places || "চেম্বার নির্ধারিত নেই"}`,
    mobileNumber: (phone) => phone ? `আমার মোবাইল নম্বর: ${phone}` : "দুঃখিত, এই মুহূর্তে মোবাইল নম্বর দেওয়া নেই।",
    scheduleInfo: (lines) => lines ? `🗓️ আমার চেম্বার সময়সূচি:\n${lines}` : "দুঃখিত, সময়সূচি এখনো দেওয়া হয়নি।",
    feeInfo: (lines) => lines ? `💰 আমার ভিজিট ফি:\n${lines}` : "দুঃখিত, ভিজিট ফি এখনো নির্ধারণ করা হয়নি।",
    fallbackResponse: (doctorName) => `আমি ${doctorName || "ডাক্তার"} বলছি। এই বিষয়ে আমি কোনো উত্তর দিতে পারব না।`,
};
export const DOCTOR_QA = [
...
export function getDoctorAnswer(normText, doctor) {
...
export const APPOINTMENT_TEXTS = {
    ...
    NO_CHAMBER: "দুঃখিত, এই ডাক্তারের কোনো চেম্বার পাওয়া যায়নি। দয়া করে পরে আবার চেষ্টা করুন অথবা নিচের 🏠 মূল মেনু বাটনে চাপ দিন।",
    ...
    NO_SLOT: "দুঃখিত, আজ ও আগামীকাল চেম্বার বন্ধ আছে। দয়া করে পরে আবার চেষ্টা করুন অথবা নিচের 🏠 মূল মেনু বাটনে চাপ দিন।",
    ...
    SUCCESS: (d) => `✅ ধন্যবাদ, ${d.patientName}! আপনার অনুরোধটি পেয়েছি।\n\n🏥 চেম্বার: ${d.chamberName}\n📅 ${d.dayLabel}\n💰 ভিজিট ফি: ${d.feeLine}\n\nআমরা শীঘ্রই আপনাকে কল করব। 📞`,
};
const BN_DIGITS = {
    "০": "0", "১": "1", "২": "2", "৩": "3", "৪": "4",
    "৫": "5", "৬": "6", "৭": "7", "৮": "8", "৯": "9",
...
export function jsDayToEnum(d) {
    return ["SUNDAY", "MONDAY", "TUESDAY", "WEDNESDAY", "THURSDAY", "FRIDAY", "SATURDAY"][d.getDay()];
}
function formatBnDate(d) {
 ...
 */
export function getAppointmentDayOptions(schedules) {
...
export function buildDayPrompt(options) {
...
export function buildChamberListText(chambers) {
    const lines = chambers.map((c, i) => `${i + 1}. ${c.name}${c.area ? ` — ${c.area}` : ""}`);
    return `${APPOINTMENT_TEXTS.ASK_CHAMBER_HEADER}\n\n${lines.join("\n")}`;
}
/** Buttons for each available chamber: chsel_<chamberId>. */
export function buildChamberButtons(chambers) {
 ...
 */
export function matchChamber(chambers, rawText, buttonId) {
    ...
    if (low.length >= 2) {
        const hit = chambers.find((c) => c.name.toLowerCase().includes(low) || low.includes(c.name.toLowerCase()));
        if (hit)
            return hit;
    }
    return null;
 ...
 */
export function schedulesForChamber(schedules, chamberId) {
    const own = (schedules || []).filter((s) => (s.chamberId || "").toLowerCase() === chamberId.toLowerCase());
    return own.length ? own : schedules || [];
}
/** Price line for the selected chamber, e.g. "নতুন: ৭০০ টাকা | পুরনো: ৬০০ টাকা". */
export function buildFeeLine(c) {
    const parts = [];
    if (Number(c.newFee) > 0)
        parts.push(`নতুন: ${c.newFee} টাকা`);
    if (Number(c.oldFee) > 0)
        parts.push(`পুরনো: ${c.oldFee} টাকা`);
    return parts.length ? parts.join(" | ") : "চেম্বারে জেনে নিন";
}
//# sourceMappingURL=doctorQA.js.map