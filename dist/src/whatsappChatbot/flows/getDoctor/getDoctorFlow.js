...
export async function handleGetDoctorFlow(phoneNumber, text, msg, session, updateSession) {
    ...
    if (session.step === "DIRECT_SEARCH" || norm.startsWith("dr-")) {
        ...
        try {
            ...
            if (updateSession)
                updateSession("GET_DOCTOR_FLOW", "APT_ASK_PROBLEM", sessionData);
            await saveConnectSession(phoneNumber, "DOCTOR", doctor.id, doctor.name, "GET_DOCTOR_FLOW", "APT_ASK_PROBLEM");
            ...
            const businessCardText = `🩺 *ডাক্তারের তথ্য ও বিবরণী*\n\n` +
                `নাম: ${doctor.name}\n` +
