...
import { getButtonId, isEntryWord, isValidProblem, } from "../../lib/session.js";
...
import { findDivision, matchDivision, matchDistrict, matchThana, matchSpeciality, fetchAvailableSpecs, fetchLocationDoctors, sendDivisionPrompt, sendDistrictPrompt, sendThanaPrompt, sendSpecialityPrompt, sendDoctorCards, sendSeeMoreButton, buildLocationDoneMessage, advancePickedSpeciality, handleAltDistrict, handleAltThana, LOCATION_TEXTS, DOCTOR_CARD_PAGE, MORE_DOCTORS_ID, SUGGEST_SPEC_ID, } from "../../lib/locationSelect.js";
import { handleGetDoctorFlow } from "../getDoctor/getDoctorFlow.js";
import { FIND_DOCTOR_TEXTS, extractConnectUsername, isConnectClick, } from "./findDoctorQA.js";
...
export async function findDoctorFlow(phoneNumber, text, msg, session, updateSession, _resetSession) {
    ...
    if (step === "ASK_THANA") {
        ...
        if (!available.length) {
            await sendWhatsAppMessage(phoneNumber, LOCATION_TEXTS.NO_DOCTORS_THANA(hit, district.name, division.name) + BACK_HINT);
            return;
        }
        const next = {
            ...data,
    ...
    if (step === "ASK_SPECIALITY") {
        ...
        if (!allowed.length) {
            await sendWhatsAppMessage(phoneNumber, LOCATION_TEXTS.NO_DOCTORS_THANA(String(data.thana), String(data.district || ""), String(data.division || "")) + BACK_HINT);
            return;
        }
        if (allowed !== stored) {
    ...
    if (step === "ASK_SUGGEST_PROBLEM") {
        ...
        const pick = await suggestCategoryFromFullList(rawText, DOCTOR_SPECIALITIES.map((s) => ({ bn: s.specialty_bn, en: s.specialty_en })));
        if (!pick) {
            const stored = Array.isArray(data.availableSpecs)
                ? data.availableSpecs
                : undefined;
