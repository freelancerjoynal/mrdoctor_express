...
export async function sendWhatsAppMessage(to, message, messageId) {
    // Fire-and-forget: typing shows while the real message is on its way.
    void sendTypingIndicator(to, messageId);
    try {
        await axios.post(`https://graph.facebook.com/v20.0/${env.PHONE_NUMBER_ID}/messages`, {
 ...
 */
export async function sendWhatsAppImage(to, imageUrl, caption, messageId) {
 ...
 * message lands.
 */
export function chatDelay(ms = 900) {
...
export async function sendTypingIndicator(_to, messageId) {
    if (!messageId)
        return;
    try {
        await axios.post(`https://graph.facebook.com/${TYPING_API_VERSION}/${env.PHONE_NUMBER_ID}/messages`, {
            messaging_product: "whatsapp",
            status: "read",
 ...
 */
export async function sendInteractiveButtons(to, bodyText, buttons, messageId) {
    // Fire-and-forget: typing shows while the real message is on its way.
    void sendTypingIndicator(to, messageId);
    try {
 ...
 */
export async function sendImageButtons(to, imageUrl, bodyText, buttons, messageId) {
    const url = (imageUrl || "").trim();
    if (!url || !/^https?:\/\//i.test(url)) {
 ...
 */
export async function sendCtaUrlButton(to, bodyText, buttonText, url, messageId) {
    const link = (url || "").trim();
...
export async function sendButtonsChunked(to, bodyText, buttons, continuationText = "⬇️ আরও অপশন নিচে:") {
    const chunks = [];
...
export function listRowTitle(s, max = 24) {
    const t = (s || "").trim();
 ...
 */
export async function sendInteractiveList(to, bodyText, buttonText, sections, options) {
...
export async function sendListChunked(to, bodyText, buttonText, rows, sectionTitle = "তালিকা", continuationText = "⬇️ আরও অপশন নিচে:") {
    for (let i = 0; i < rows.length; i += 10) {
        await sendInteractiveList(to, i === 0 ? bodyText : continuationText, buttonText, [{ title: sectionTitle, rows: rows.slice(i, i + 10) }]);
    }
}
