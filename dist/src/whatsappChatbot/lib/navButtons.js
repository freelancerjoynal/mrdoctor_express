 ...
 */
export function isBackClick(raw, buttonId) {
    if ((buttonId || "").trim() === BACK_BUTTON.id)
        return true;
    const t = (raw || "").trim().toLowerCase();
    if (t === BACK_BUTTON.title)
        return true; // legacy tapped button title
 ...
 */
export async function resendStepPrompt(phoneNumber, session) {
        ...
        if (session.flow === "FIND_HOSPITAL_FLOW") {
            switch (session.step) {
                ...
                case "SELECT_HOSPITAL": {
                    ...
                    const ids = Array.isArray(data.hospitalCandidates)
                        ? data.hospitalCandidates.slice(0, 5)
                        : [];
                    await sendWhatsAppMessage(phoneNumber, HOSPITAL_TEXTS.HOSPITAL_LIST_HEADER(data.location || "আপনার এলাকা"));
                ...
                case "SELECT_DEPT": {
                    ...
                    await sendButtonsChunked(phoneNumber, HOSPITAL_TEXTS.ASK_DEPT(data.hospitalName || "হসপিটাল") +
                        "\n" +
        ...
        if (session.flow === "FIND_DOCTOR_FLOW" || session.flow === "AI_DOCTOR_FLOW") {
            switch (session.step) {
                ...
                case "ASK_AREA": {
                    ...
                    await sendWhatsAppMessage(phoneNumber, (why ? FIND_DOCTOR_TEXTS.ASK_AREA(dept, why) : FIND_DOCTOR_TEXTS.ASK_AREA_RETRY) + BACK_HINT);
                    return;
                }
                case "SELECT_DOCTOR":
                case "CONFIRM_RESULT":
        ...
        if (session.flow === "GET_DOCTOR_FLOW") {
            switch (session.step) {
                ...
                case "APT_ASK_DAY": {
                    ...
                    await sendInteractiveButtons(phoneNumber, buildDayPrompt(options), options.map((o) => ({ id: o.buttonId, title: o.buttonTitle })));
                    return;
                }
                case "APT_ASK_NAME":
            ...
            if (data.doctorId) {
                await sendButtonsChunked(phoneNumber, "আপনাকে কীভাবে সাহায্য করতে পারি নিচে থেকে বেছে নিন:", [
