...
export function findNearestAreas(lat, lng, limit = 3) {
 ...
 */
export function findNearestByName(name, limit = 3) {
    const q = (name || "").toLowerCase().trim();
    if (!q)
        return [];
    const hit = AREAS.find((a) => a.nameBn === name.trim() || q.includes(a.nameBn) || a.aliases.some((al) => q.includes(al)));
    if (!hit)
        return [];
    return [...AREAS]
...
//# sourceMappingURL=nearbyAreas.js.map