...
const profileRoles = ['SUPER_ADMIN', 'ADMIN_MANAGER', 'DOCTOR', 'DOCTOR_STAFF', 'BUSINESS_OWNER', 'HOSPITAL', 'HOSPITAL_STAFF'];
usersProfileRouter.get('/', protectedRoute(...profileRoles), getUserProfile);
// PATCH|PUT /api/users/profile — own name/password + (DOCTOR) full doctor fields; email/username immutable.
usersProfileRouter.patch('/', protectedRoute(...profileRoles), updateUserProfile);
usersProfileRouter.put('/', protectedRoute(...profileRoles), updateUserProfile);
