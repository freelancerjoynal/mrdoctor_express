 ...
 */
export function parseSkipMap(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return {};
    const now = new Date();
    const out = {};
    for (const [k, v] of Object.entries(raw)) {
        if (!/^\d+$/.test(k) || typeof v !== 'string')
            continue;
        const t = new Date(v);
...
function isLiveStale(liveUpdatedAt) {
 ...
 */
function hardStopReason(queueLength, liveUpdatedAt) {
    if (queueLength === 0)
        return 'queue-empty';
    if (liveUpdatedAt && !isSameLocalDay(liveUpdatedAt, new Date()))
        return 'date-rollover';
    return null;
}
/** Force a doctor's live OFF (best-effort notify so TVs/dashboards flip). */
async function autoStopLive(doctorId, reason) {
...
async function todayQueue(doctorId) {
 ...
 */
function toLiveBreak(live, reason, until) {
    if (!live || !reason)
        return null;
...
function snapshot(live, liveCurrentSerial, liveUpdatedAt, queue, skipMap = {}, liveBreak = null) {
...
export async function getSerialLiveStatus(caller) {
...
export async function startSerialLive(caller) {
    const doctorId = await resolveOwnDoctorId(caller);
    const queue = await todayQueue(doctorId);
    // No point glowing LIVE on an empty list (and no server load for nothing).
    if (queue.length === 0)
        throw new Error('NO_APPOINTMENTS');
...
export async function stopSerialLive(caller) {
    const doctorId = await resolveOwnDoctorId(caller);
    const doctor = await prisma.doctor.update({
        where: { id: doctorId },
 ...
 */
export async function heartbeatLive(caller) {
    const doctorId = await resolveOwnDoctorId(caller);
    const doctor = await prisma.doctor.findUnique({
        where: { id: doctorId },
 ...
 */
export async function stopLivesForUser(userId) {
    try {
        ...
        const ids = new Set();
        const own = user?.doctorProfile?.id;
        const staff = user?.staffDoctor?.id;
        if (own)
            ids.add(own);
        if (staff)
            ids.add(staff);
 ...
 */
export async function autoStopIfQueueEmpty(doctorId) {
    try {
        ...
        if (n === 0)
            await autoStopLive(doctorId, 'queue-empty');
    }
    catch (error) {
 ...
 */
export async function sweepStaleLives() {
    ...
    if (stopped > 0)
        console.log(`[SerialLive] sweep stopped ${stopped} stale live(s)`);
    return stopped;
}
/**
 ...
 */
export async function startLiveBreak(caller, reasonRaw, minutesRaw) {
    const doctorId = await resolveOwnDoctorId(caller);
    const doctor = await prisma.doctor.findUnique({
        where: { id: doctorId },
    ...
    const liveBreak = { reason: reason || 'বিরতি', endsAt: new Date(Date.now() + mins * 60000) };
...
export async function endLiveBreak(caller) {
    const doctorId = await resolveOwnDoctorId(caller);
    const doctor = await prisma.doctor.findUnique({
        where: { id: doctorId },
        select: { id: true, serialLive: true, liveCurrentSerial: true, liveUpdatedAt: true, liveSkippedAt: true },
 ...
 */
export async function skipCurrentSerial(caller) {
    const doctorId = await resolveOwnDoctorId(caller);
    const doctor = await prisma.doctor.findUnique({
        where: { id: doctorId },
 ...
 */
async function autoReleaseIfFewLeft(doctorId, live, queue, skipMap, missedCount) {
    if (!live || missedCount === 0)
        return skipMap;
    if (queue.length - missedCount > 2)
        return skipMap;
    const inQueue = new Set(queue.map((q) => String(q.serial)));
    const pruned = {};
    for (const [k, v] of Object.entries(skipMap)) {
 ...
 */
export async function recallSerial(caller, serialRaw) {
    ...
    const skipMap = parseSkipMap(doctor.liveSkippedAt);
    const pinned = doctor.liveCurrentSerial != null
        ? (queue.find((q) => q.serial === doctor.liveCurrentSerial) ?? null)
        : null;
    const current = pinned ?? queue[0] ?? null;
    if (current && target.serial === current.serial)
        throw new Error('NOT_MISSED');
    ...
    if (ts) {
        ...
        if (elapsed < RECALL_COOLDOWN_MS) {
            const err = new Error('RECALL_COOLDOWN');
    ...
    const pruned = {};
    for (const [k, v] of Object.entries(skipMap)) {
        if (k !== String(serial) && inQueue.has(k))
            pruned[k] = v;
    }
    const updated = await prisma.doctor.update({
        where: { id: doctorId },
 ...
 */
export async function advanceSerialLiveAfterServe(doctorId, servedSerial) {
    try {
        const doctor = await prisma.doctor.findUnique({
            where: { id: doctorId },
...
//# sourceMappingURL=serialLiveService.js.map