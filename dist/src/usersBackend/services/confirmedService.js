...
async function ownershipFilter(caller, doctorUsername) {
    ...
    if (caller.role === 'HOSPITAL_STAFF') {
        ...
        const hospitalId = own?.staffHospitalId;
        if (!hospitalId)
            throw new Error('NO_HOSPITAL_PROFILE');
        return { hospitalId };
    }
 ...
 */
async function applyDoctorIdFilter(caller, owned, doctorId) {
    const id = doctorId?.trim();
    if (!id)
        return owned;
    if (caller.role === 'HOSPITAL' || caller.role === 'HOSPITAL_STAFF') {
        const hospitalId = owned.hospitalId;
        if (!hospitalId)
            throw new Error('NO_HOSPITAL_PROFILE');
...
export async function listConfirmed(caller, filters) {
    ...
    const where = { ...owned, appointmentDate: { gte, lt } };
...
export async function getConfirmedCounts(caller, doctorUsername, doctorId) {
...
export async function getServedCounts(caller, doctorUsername, doctorId) {
    const baseOwned = await ownershipFilter(caller, doctorUsername);
    const owned = await applyDoctorIdFilter(caller, baseOwned, doctorId);
    ...
    let scope = { ...owned };
    if (owned.hospitalId) {
        const narrowedDoctorId = owned.doctorId;
        if (narrowedDoctorId) {
            scope = { doctorId: narrowedDoctorId };
        }
        else {
            const chambers = await prisma.chamber.findMany({
                where: { hospitalId: owned.hospitalId },
                select: { doctorId: true },
            });
            const doctorIds = [...new Set(chambers.map((c) => c.doctorId).filter(Boolean))];
 ...
 */
async function assertCanServe(caller) {
...
export async function completeConfirmed(caller, id) {
    ...
    if (startOf(row.appointmentDate) > startOf(new Date()))
        throw new Error('FUTURE_SERVE');
...
export async function listServed(caller, filters) {
    const baseOwned = await ownershipFilter(caller, filters.doctorUsername);
    const owned = await applyDoctorIdFilter(caller, baseOwned, filters.doctorId);
    const { gte, lt } = rangeBounds(filters.range);
    let where = { ...owned, appointmentDate: { gte, lt } };
    if (filters.bookingType === 'ONLINE' || filters.bookingType === 'OFFLINE') {
        where.bookingType = filters.bookingType;
    }
    if (owned.hospitalId) {
...
export async function updateConfirmed(caller, id, input) {
    const row = await ownedRow(caller, id);
    if (row.status === 'CANCELLED')
        throw new Error('ALREADY_CANCELLED');
    if ((caller.role === 'HOSPITAL' || caller.role === 'HOSPITAL_STAFF') &&
    ...
    if (input.appointmentDate !== undefined && input.appointmentDate !== null && String(input.appointmentDate).trim() !== '') {
        ...
        if (isoDayOf(newDate) !== isoDayOf(row.appointmentDate))
            throw new Error('DATE_IMMUTABLE');
    }
    const data = {};
    if (input.patientName !== undefined)
        data.patientName = cleanRowName(input.patientName);
    ...
    if (input.collectionAmount !== undefined) {
        if (row.bookingType !== 'OFFLINE')
            throw new Error('AMOUNT_NOT_EDITABLE');
        const n = typeof input.collectionAmount === 'string' && input.collectionAmount.trim() !== ''
 ...
 */
export async function deleteOfflineBooking(caller, id, reason) {
    const row = await ownedRow(caller, id);
    if (row.bookingType !== 'OFFLINE')
        throw new Error('ONLINE_DELETE_FORBIDDEN');
 ...
 */
async function rearrangeDaySerials(doctorId, appointmentDate) {
    ...
    if (moves.length === 0)
        return;
    await prisma.$transaction(moves.map((m) => prisma.confirmedAppointment.update({ where: { id: m.id }, data: { serial: -m.to } })));
    await prisma.$transaction(moves.map((m) => prisma.confirmedAppointment.update({ where: { id: m.id }, data: { serial: m.to } })));
}
/**
 ...
 */
export async function requestOnlineCancel(caller, id, reason) {
    const row = await ownedRow(caller, id);
...
//# sourceMappingURL=confirmedService.js.map