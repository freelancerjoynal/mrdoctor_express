...
function rangeBounds(range, date) {
...
async function resolveDoctorIds(caller, doctorUsername, doctorId) {
...
function amountOf(r) {
    const c = r.collectionAmount;
    const p = r.paymentAmount;
...
export async function getStaffCollections(caller, opts) {
    ...
    const base = { doctorId: { in: doctorIds }, bookingType: 'OFFLINE', appointmentDate: { gte, lt } };
    ...
    if (missing.length > 0) {
        ...
        for (const u of users) {
            ...
            if (b)
                b.name = u.name?.trim() || u.email.split('@')[0] || 'স্টাফ';
        }
    }
    ...
    if (caller.role === 'DOCTOR' || caller.role === 'DOCTOR_STAFF') {
        ...
        if (knownIds.length > 0) {
            ...
            for (const t of takers) {
                if (t.role !== 'HOSPITAL' && t.role !== 'HOSPITAL_STAFF')
                    continue;
                const hid = t.role === 'HOSPITAL'
                    ? t.hospitalProfile?.id
                    : t.staffHospitalId;
                if (hid)
                    hospitalIds.add(hid);
            }
            ...
            if (hospitalIds.size > 0) {
                ...
                for (const h of hospitals)
                    hospitalNameById.set(h.id, h.name);
                // Fallback to profile snapshots when the hospital row is missing.
                for (const t of takers) {
                    if (t.role === 'HOSPITAL') {
                        const hid = t.hospitalProfile?.id;
                        const hname = t.hospitalProfile?.name;
                        if (hid && !hospitalNameById.get(hid) && hname)
                            hospitalNameById.set(hid, hname);
                    }
                    else if (t.role === 'HOSPITAL_STAFF') {
                        const hid = t.staffHospitalId;
                        const hname = t.staffHospital?.name;
                        if (hid && !hospitalNameById.get(hid) && hname)
                            hospitalNameById.set(hid, hname);
                    }
                }
 ...
 * rows transferred.
 */
export async function getLocalMonthlyCounts(caller, opts = {}) {
    const doctorIds = await resolveDoctorIds(caller, opts.doctorUsername, opts.doctorId);
    const now = new Date();
    const bounds = [];
    for (let i = 11; i >= 0; i--) {
    ...
    const base = { doctorId: { in: doctorIds }, bookingType: 'OFFLINE' };
    const counts = await Promise.all(bounds.map(async (b) => {
...
export async function listStaffRows(caller, opts) {
    const doctorIds = await resolveDoctorIds(caller, opts.doctorUsername, opts.doctorId);
    const { gte, lt } = rangeBounds(opts.range, opts.date);
    ...
    const servedDoctors = servedDoctorIds.length > 0
    ...
    const confirmedRows = confirmed.map((r) => toRow(r, false));
    const servedRows = served.map((r) => {
        const info = servedDoctorMap.get(r.doctorId);
        return toRow({
            ...r,
            doctor: info ?? null,
        }, true);
...
//# sourceMappingURL=staffCollectionService.js.map