...
function chamberOwningDay(schedules, dayOfWeek) {
    const hit = schedules.find((s) => s.chamberId && String(s.dayOfWeek).toUpperCase() === dayOfWeek);
    return hit?.chamberId ?? null;
}
/** Who is taking this booking (cash tracking): user id + display name snapshot. */
async function resolveTaker(caller) {
...
async function resolveHospital(caller) {
    ...
    if (caller.role === 'HOSPITAL_STAFF') {
        ...
        const hospital = own?.staffHospital;
        if (!hospital)
            throw new Error('NO_HOSPITAL_PROFILE');
        return hospital;
    }
...
async function resolveHospitalDoctor(hospitalId, rawDoctorId) {
...
export async function hospitalDoctorsToday(hospitalId) {
    const now = new Date();
    ...
    for (const c of chambers)
        if (c.doctorId)
            linked.add(c.doctorId);
    for (const s of schedules)
        if (s.doctorId)
            linked.add(s.doctorId);
    if (linked.size === 0)
        return [];
    const runningToday = new Set(schedules
...
export async function createLocalBooking(caller, input) {
    ...
    if (type !== 'NEW' && type !== 'RENEW')
        throw new Error('INVALID_PATIENT_TYPE');
    ...
    if (input.date !== undefined && input.date !== null && String(input.date).trim() !== '') {
        ...
        if (!parsed || isoDay(parsed) !== isoDay(appointmentDate))
    ...
    const requestedChamberId = typeof input.chamberId === 'string' && input.chamberId.trim() ? input.chamberId.trim() : null;
    ...
    if (effectiveChamberId && (!chamber || chamber.doctorId !== doctor.id))
        throw new Error('INVALID_CHAMBER');
    // Hospital desk: the chamber must belong to this hospital.
    if (isHospitalDesk && chamber && chamber.hospitalId !== hospital.id)
        throw new Error('INVALID_CHAMBER');
    if (chamber) {
    ...
    if (typeof bookingId === 'string' && bookingId)
        await linkCreditRef(charge.ledgerId, bookingId);
    let smsSent = false;
    try {
        ...
        const place = hospitalWords.length
            ...
            : (chamberName || '').trim();
        const text = `প্রিয় ${patientName}, ${doctor.name}-এর সিরিয়াল-${serial} নিশ্চিত` +
            (place ? `, ${place}` : '') +
 ...
 */
export async function getLocalBookingOptions(caller) {
...
//# sourceMappingURL=localBookingService.js.map