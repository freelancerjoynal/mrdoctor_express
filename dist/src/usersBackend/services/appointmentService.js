...
export const APPOINTMENT_STATUSES = ['PENDING', 'CONFIRMED', 'DONE', 'CANCELLED'];
async function resolveDoctorId(caller) {
...
export async function chamberFeeMap(doctorId) {
...
export function feeOf(row, fees) {
    if (!row.chamberId)
        return 0;
...
export async function listAppointments(caller, filters) {
    const owned = await ownershipFilter(caller);
    const where = { ...owned };
    if (APPOINTMENT_STATUSES.includes(filters.status ?? '')) {
    ...
    const doctorId = where.doctorId ?? owned.doctorId;
...
export async function listTodayAppointments(caller) {
    const owned = await ownershipFilter(caller);
...
function bucket(rows, fees) {
    let total = 0;
    let newCount = 0;
    let renewCount = 0;
    for (const r of rows) {
        total += feeOf(r, fees);
        if (r.patientType === 'RENEW')
            renewCount += 1;
        else
            newCount += 1;
    }
...
function weeklyBucket(rows, fees, from, to) {
    let total = 0;
    let doneCount = 0;
    let newCount = 0;
    let renewCount = 0;
...
export async function getAppointmentSummary(caller, opts = {}) {
    const owned = await ownershipFilter(caller);
    let doctorId = owned.doctorId;
    ...
    if (isStaff) {
        if (customFrom < weekAgo)
            customFrom = new Date(weekAgo);
        if (customToIncl > today)
            customToIncl = new Date(today);
    }
...
function pickPaging(filters) {
    const page = Math.max(1, filters.page || 1);
    const limit = Math.min(50, Math.max(1, filters.limit || 20));
    return { page, limit };
}
export async function updateAppointmentStatus(caller, id, status) {
    ...
    const denied = Object.entries(owned).some(([key, value]) => existing[key] !== value);
    if (denied)
        throw new Error('FORBIDDEN');
