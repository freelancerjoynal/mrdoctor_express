...
export async function getProfileData(caller, targetUserId) {
...
function cleanRequiredText(raw, min, max, code = 'INVALID_DOCTOR_FIELD') {
    if (typeof raw !== 'string')
...
function cleanOptionalText(raw, max, code = 'INVALID_DOCTOR_FIELD') {
    if (raw === undefined)
        return undefined;
    if (raw === null)
        return null;
...
function cleanOptionalLong(raw, max, code = 'INVALID_DOCTOR_FIELD') {
    if (raw === undefined)
        return undefined;
    if (raw === null)
        return null;
...
function parseDoctorPayload(input) {
    if (input === undefined || input === null)
        return {};
    if (typeof input !== 'object' || Array.isArray(input))
        throw new Error('INVALID_DOCTOR_FIELD');
    const d = input;
    ...
    if (typeof d.username === 'string' && d.username.trim().length > 0)
    ...
    if (d.id !== undefined && d.id !== null && String(d.id).trim().length > 0)
    ...
    const out = {};
    if (d.name !== undefined)
        out.name = cleanRequiredText(d.name, 2, 80);
    if (d.name_en !== undefined)
        out.name_en = cleanOptionalText(d.name_en, 80) ?? null;
    if (d.degree !== undefined)
        out.degree = cleanRequiredText(d.degree, 2, 200);
    if (d.degree_en !== undefined)
        out.degree_en = cleanOptionalText(d.degree_en, 200) ?? null;
    if (d.speciality !== undefined)
        out.speciality = cleanRequiredText(d.speciality, 2, 120);
    if (d.speciality_en !== undefined)
        out.speciality_en = cleanOptionalText(d.speciality_en, 120) ?? null;
    if (d.tagline !== undefined)
        out.tagline = cleanOptionalText(d.tagline, 200) ?? null;
    if (d.tagline_en !== undefined)
        out.tagline_en = cleanOptionalText(d.tagline_en, 200) ?? null;
    if (d.bio !== undefined)
        out.bio = cleanOptionalLong(d.bio, 5000) ?? null;
    if (d.bio_en !== undefined)
        out.bio_en = cleanOptionalLong(d.bio_en, 5000) ?? null;
    ...
    if (d.whatsappId !== undefined)
        out.whatsappId = cleanOptionalText(d.whatsappId, 100) ?? null;
    if (d.whatsappAccessToken !== undefined)
        out.whatsappAccessToken = cleanOptionalText(d.whatsappAccessToken, 1000) ?? null;
    if (d.templateName !== undefined)
    ...
    if (d.bmdcNumber !== undefined)
        out.bmdcNumber = cleanOptionalText(d.bmdcNumber, 50) ?? null;
    ...
    if (d.startedYear !== undefined) {
        if (d.startedYear === null)
            out.startedYear = null;
        else if (typeof d.startedYear === 'string' && !d.startedYear.trim())
            out.startedYear = null;
        else {
...
export async function updateProfileData(caller, input) {
    ...
    let picture;
    if (wantsPicture)
        picture = cleanOptionalText(input.profilePicture, 500, 'INVALID_PICTURE') ?? null;
    let hashedPassword;
    if (wantsPassword) {
    ...
    const effectiveUserName = caller.role === 'DOCTOR' ? (doctorData.name ?? name) : name;
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (caller.role === 'DOCTOR') {
            ...
            if (doctorMirrorName !== undefined)
                data.name = doctorMirrorName;
