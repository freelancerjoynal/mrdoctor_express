import { listAppointments, listTodayAppointments, getAppointmentSummary, updateAppointmentStatus, } from '../services/appointmentService.js';
...
import { listConfirmed, } from '../services/confirmedService.js';
import { getConfirmedCounts, getServedCounts } from '../services/confirmedService.js';
import { completeConfirmed, updateConfirmed, deleteOfflineBooking, requestOnlineCancel, listServed, } from '../services/confirmedService.js';
import { getStaffCollections, listStaffRows, getLocalMonthlyCounts, } from '../services/staffCollectionService.js';
...
export const showTodayAppointments = async (req, res) => {
    try {
...
export const showAppointmentSummary = async (req, res) => {
    try {
...
export const patchUserAppointment = async (req, res) => {
    try {
        const updated = await updateAppointmentStatus(callerOf(req), req.params.id, req.body?.status);
...
export const showLocalBookingOptions = async (req, res) => {
    try {
...
export const showCollectionSummary = async (req, res) => {
    try {
...
export const showLifetimeBalance = async (req, res) => {
    try {
...
export const showCollectionDays = async (req, res) => {
    try {
...
export const showCollectionWeek = async (req, res) => {
    try {
...
export const showCollectionDoctors = async (req, res) => {
    try {
...
export const createLocalBookingAppointment = async (req, res) => {
    try {
        ...
        if (error.message === 'NO_DOCTOR_PROFILE' || error.message === 'NO_HOSPITAL_PROFILE')
            return res.status(404).json({ error: 'No profile linked to this account' });
        if (error.message === 'DOCTOR_REQUIRED')
            return res.status(400).json({ error: 'ডাক্তার বেছে নিন।' });
        if (error.message === 'DOCTOR_NOT_FOUND')
            return res.status(404).json({ error: 'ডাক্তার পাওয়া যায়নি।' });
        if (error.message === 'DOCTOR_NOT_IN_HOSPITAL')
            return res.status(400).json({ error: 'এই ডাক্তার এই হাসপাতালের নন।' });
        if (error.message === 'INVALID_NAME')
            return res.status(400).json({ error: 'রোগীর নাম দিন (২–৮০ অক্ষর)।' });
        ...
        if (error.message === 'INVALID_AMOUNT')
            return res.status(400).json({ error: 'সঠিক আদায়ের টাকা দিন।' });
        if (error.message === 'INVALID_DATE')
            return res.status(400).json({ error: 'শুধু আজকের তারিখে বুকিং করা যাবে।' });
        if (error.message === 'CLOSED_DAY')
            return res.status(400).json({ error: 'আজ চেম্বার বন্ধ আছে।' });
        if (error.message === 'INVALID_AGE')
            return res.status(400).json({ error: 'সঠিক বয়স দিন।' });
        if (error.message === 'INVALID_CHAMBER')
            return res.status(400).json({ error: 'চেম্বার সঠিক নয়।' });
...
export const listConfirmedAppointments = async (req, res) => {
    try {
        ...
        const range = rawRange === 'tomorrow' || rawRange === 'last30' ? rawRange : 'today';
        const bookingType = rawType === 'ONLINE' || rawType === 'OFFLINE' ? rawType : 'ALL';
...
export const showConfirmedCounts = async (req, res) => {
    try {
        const data = await getConfirmedCounts(callerOf(req), parseOptional(req, 'doctorUsername'), parseOptional(req, 'doctorId'));
        return res.json({ backend: 'usersBackend', data });
    }
    catch (error) {
        if (error.message === 'FORBIDDEN')
...
export const showServedCounts = async (req, res) => {
    try {
        const data = await getServedCounts(callerOf(req), parseOptional(req, 'doctorUsername'), parseOptional(req, 'doctorId'));
        return res.json({ backend: 'usersBackend', data });
    }
    catch (error) {
        if (error.message === 'FORBIDDEN')
...
export const patchConfirmedAppointment = async (req, res) => {
    try {
        const id = req.params.id;
        const body = req.body ?? {};
        const updated = body?.status === 'DONE'
...
export const deleteConfirmedAppointment = async (req, res) => {
    try {
        await deleteOfflineBooking(callerOf(req), req.params.id, req.body?.reason);
...
export const postCancelRequest = async (req, res) => {
    try {
        await requestOnlineCancel(callerOf(req), req.params.id, req.body?.reason);
        return res.status(201).json({
            backend: 'usersBackend',
            data: { requested: true },
...
export const listServedAppointments = async (req, res) => {
    try {
        const rawRange = parseOptional(req, 'range');
        const rawType = parseOptional(req, 'bookingType');
        const range = rawRange === 'tomorrow' || rawRange === 'last30' ? rawRange : 'today';
...
export const showStaffCollections = async (req, res) => {
    try {
        const rawRange = parseOptional(req, 'range');
        const range = rawRange === 'tomorrow' || rawRange === 'yesterday' || rawRange === 'last30' ? rawRange : 'today';
...
export const showLocalMonthlyCounts = async (req, res) => {
    try {
...
export const showStaffCollectionRows = async (req, res) => {
    try {
        const rawRange = parseOptional(req, 'range');
        const range = rawRange === 'tomorrow' || rawRange === 'yesterday' || rawRange === 'last30' ? rawRange : 'today';
...
function confirmedActionError(res, error) {
    ...
    if (msg === 'NOT_ONLINE')
        return res.status(400).json({ error: 'এটি অনলাইন বুকিং নয়।' });
    if (msg === 'ALREADY_CANCELLED')
        return res.status(400).json({ error: 'বুকিংটি আগেই বাতিল হয়েছে।' });
    if (msg === 'FUTURE_SERVE')
    ...
    if (msg === 'NOTHING_TO_UPDATE')
        return res.status(400).json({ error: 'বদলানোর মতো কিছু নেই।' });
    if (msg === 'INVALID_NAME')
        return res.status(400).json({ error: 'রোগীর নাম দিন (২–৮০ অক্ষর)।' });
    if (msg === 'INVALID_PHONE')
        return res.status(400).json({ error: 'সঠিক মোবাইল নম্বর দিন (01XXXXXXXXX)।' });
    if (msg === 'INVALID_AMOUNT')
        return res.status(400).json({ error: 'সঠিক টাকা দিন।' });
    if (msg === 'INVALID_DATE')
        return res.status(400).json({ error: 'শুধু আজকের তারিখে বুকিং করা যাবে।' });
    if (msg === 'CLOSED_DAY')
        return res.status(400).json({ error: 'আজ চেম্বার বন্ধ আছে।' });
