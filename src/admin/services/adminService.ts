...
export const getAdminOverview = async () => {
  ...
  const users: Record<string, number> = { total: usersTotal };
...
// ------------------------------------------------------------------
// Directory: filter doctors/hospitals by location, then drill in.
// Doctors carry no location columns themselves — their location comes
// from their chambers (division/district/thana on chambers).
// ------------------------------------------------------------------
...
export const getLocationOptions = async () => {
  ...
  const districtByDivision = new Map<string, Set<string>>();
  const thanaByDistrict = new Map<string, Set<string>>();
  for (const r of [...hospitals, ...chambers]) {
    ...
    if (div) {
      divisions.add(div);
      if (dis) {
        if (!districtByDivision.has(div)) districtByDivision.set(div, new Set());
        districtByDivision.get(div)!.add(dis);
        if (tha) {
          if (!thanaByDistrict.has(dis)) thanaByDistrict.set(dis, new Set());
...
export const listDirectory = async (filter: DirectoryFilter) => {
  ...
  const chamberLoc: Record<string, string> = {
    ...(division ? { division } : {}),
    ...(district ? { district } : {}),
    ...(thana ? { thana } : {}),
  };
...
const rowAmount = (r: { collectionAmount: number | null; paymentAmount: number | null }) =>
  (r.collectionAmount ?? 0) + (r.paymentAmount ?? 0);

export const getDoctorOverview = async (id: string) => {
...
export const getHospitalOverview = async (id: string) => {
