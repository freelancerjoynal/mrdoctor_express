...
export const runSeedAdminUser = async (input: SuperAdminSeedInput) => {
  const email = input.email?.trim().toLowerCase() ?? '';
  const name = input.name?.trim() || (input.role === 'ADMIN_MANAGER' ? 'Admin Manager' : 'Super Admin');
  const role: AdminSeedRole = input.role ?? 'SUPER_ADMIN';

  if (role !== 'SUPER_ADMIN' && role !== 'ADMIN_MANAGER') throw new Error('INVALID_ROLE');
...
export const runSeedSuperAdmin = async (input: SuperAdminSeedInput) =>
  runSeedAdminUser({ ...input, role: input.role ?? 'SUPER_ADMIN' });

/** Convenience wrapper — seeds an ADMIN_MANAGER. */
export const runSeedAdminManager = async (input: SuperAdminSeedInput) =>
  runSeedAdminUser({ ...input, role: 'ADMIN_MANAGER' });
