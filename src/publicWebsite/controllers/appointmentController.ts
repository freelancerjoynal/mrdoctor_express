...
export const showAppointmentOptions = async (req: Request, res: Response) => {
  try {
    const options = await getAppointmentOptions(req.params.username as string);
...
export const postAppointment = async (req: Request, res: Response) => {
  try {
    ...
    if (msg === 'INVALID_NAME') return res.status(400).json({ error: 'রোগীর নাম দিন (কমপক্ষে ৩ অক্ষর)।' });
    if (msg === 'INVALID_PATIENT_TYPE') return res.status(400).json({ error: 'রোগীর ধরন বেছে নিন (নতুন / পুরনো)।' });
    if (msg === 'INVALID_PHONE') return res.status(400).json({ error: 'সঠিক মোবাইল নম্বর দিন (যেমন: 01XXXXXXXXX)।' });
    if (msg === 'INVALID_PROBLEM') return res.status(400).json({ error: 'সমস্যাটি একটু বিস্তারিত লিখুন (কমপক্ষে ৩ অক্ষর)।' });
    if (msg === 'INVALID_AGE') return res.status(400).json({ error: 'বয়স সংখ্যায় লিখুন (যেমন: ৩৫)।' });
    if (msg === 'INVALID_WEIGHT') return res.status(400).json({ error: 'ওজন সংখ্যায় লিখুন (যেমন: ৬৫)।' });
    if (msg === 'INVALID_CHAMBER') return res.status(400).json({ error: 'সঠিক চেম্বার বেছে নিন।' });
    if (msg === 'CHAMBER_REQUIRED') return res.status(400).json({ error: 'কোন চেম্বারে দেখাতে চান বেছে নিন।' });
