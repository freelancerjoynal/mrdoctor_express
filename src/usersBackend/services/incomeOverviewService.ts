...
export async function getIncomeOverview(opts: {
  ...
  if (!Number.isInteger(take) || take < 1 || take > 200) take = 20;
  ...
  // 1) Page entities.
  type Entity = { id: string; name: string; sub: string | null };
  let entities: Entity[];
  if (scope === 'doctors') {
  ...
  const byEntity = new Map<string, IncomeRow>();
  ...
  const byDay = new Map<string, IncomeDayPoint>();
  ...
  let days: IncomeDayPoint[] | null = null;
  if (period !== 'daily') {
    days = [];
