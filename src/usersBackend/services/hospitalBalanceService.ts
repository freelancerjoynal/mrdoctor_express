...
function emptyRange(): RangeBucket {
...
async function resolveScope(caller: BalanceCaller, hospitalId?: string): Promise<Scope> {
  if (isAdminRole(caller.role)) {
    const hid = hospitalId?.trim();
    if (!hid) throw new Error('HOSPITAL_REQUIRED');
  ...
  if (caller.role === 'HOSPITAL' || caller.role === 'HOSPITAL_STAFF') {
    ...
    const hid =
      caller.role === 'HOSPITAL'
        ? (own as { hospitalProfile?: { id: string; name: string; createdAt: Date } | null } | null)?.hospitalProfile?.id
        : (own as { staffHospitalId?: string | null } | null)?.staffHospitalId;
    if (!hid) throw new Error('NO_HOSPITAL_PROFILE');
    const hospital = await prisma.hospital.findUnique({
      where: { id: hid },
      select: { id: true, name: true, createdAt: true },
    });
...
async function collectDoctorIds(hospitalId: string): Promise<string[]> {
...
function servedWhere(scope: Scope, extra: Record<string, unknown>): Record<string, unknown> {
  if (scope.doctorIds.length === 0) return { hospitalId: scope.hospitalId, ...extra };
  return {
    OR: [
...
async function onlineAggWhere(scope: Scope, gte: Date | null, lt: Date | null): Promise<OnlineAgg> {
 ...
 */
async function rangeAgg(scope: Scope, gte: Date, lt: Date): Promise<RangeBucket> {
...
async function ensureOpening(scope: Scope, today: Date): Promise<void> {
 ...
 */
async function closeDueDays(scope: Scope, today: Date): Promise<number> {
...
async function ledgerSums(hospitalId: string): Promise<OnlineAgg> {
...
export interface HospitalBalanceSummary {
  ...
  today: { date: string } & RangeBucket;
  week: { from: string; to: string } & RangeBucket;
  month: { year: number; month: number; name: string; from: string; to: string } & RangeBucket;
 ...
 */
export async function getHospitalBalanceSummary(
...
export async function listOnlineDays(
  caller: BalanceCaller,
  opts: { hospitalId?: string; page?: number; limit?: number } = {},
): Promise<DayList> {
  const scope = await resolveScope(caller, opts.hospitalId);
...
export async function closeLedgerNow(
  caller: BalanceCaller,
  opts: { hospitalId?: string } = {},
...
export async function recordOnlineServe(input: {
  ...
  if (input.bookingType !== 'ONLINE') return;
  ...
  if (input.hospitalId) {
    ...
    const ids = new Set<string>();
    for (const c of chambers) if (c.hospitalId) ids.add(c.hospitalId);
    for (const s of schedules) {
      const hid = (s as { hospitalId?: string | null }).hospitalId;
      if (hid) ids.add(hid);
    }
    hospitalIds = [...ids];
  }
  ...
  await Promise.all(
    hospitalIds.map((hid) =>
      prisma.hospitalOnlineDay.upsert({
...
export async function listHospitalPayouts(
  caller: BalanceCaller,
  opts: { hospitalId?: string; page?: number; limit?: number } = {},
): Promise<PayoutList> {
  const scope = await resolveScope(caller, opts.hospitalId);
...
export async function searchHospitalsForPayout(caller: BalanceCaller, search?: string) {
...
export async function createHospitalPayout(
  caller: BalanceCaller,
  input: { hospitalId?: string; amount?: unknown; method?: unknown; note?: unknown },
) {
  if (!isAdminRole(caller.role)) throw new Error('FORBIDDEN');
